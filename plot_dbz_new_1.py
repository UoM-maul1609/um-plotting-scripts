...
cube = iris.load_cube("20051130T1200Z_Darwin_km1p5_RAL3p2_504p4_py.pp",level)
...
def main():
	...
	dbz = ax.contourf(lons,lats,cube.data[32,:,:],levels=dbz_range,cmap="Spectral_r")
	...
	#经纬度线刻度，以2°为间隔值计算刻度值
	ax.set_xticks(np.arange(lon_min, lon_max+1, step=2), crs=crs)
	ax.set_yticks(np.arange(lat_min, lat_max+1, step=2), crs=crs)
	#设置刻度格式为经纬度格式
